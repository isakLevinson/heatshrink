#define DEF_DBG_MODULE	DBG_MODULE_COMPRESS
...
heatshrink_encoder *heatshrink_encoder_alloc(uint8_t window_sz2,
    ...
    TRACE("-- allocated encoder with buffer size of %d (%u byte input size)\n",
...
HSE_sink_res heatshrink_encoder_sink(heatshrink_encoder *hse,
    ...
    *input_size = cp_sz;
    ...
    TRACE("-- sunk %u bytes (of %d) into encoder at %d, input buffer now has %u\n",
        cp_sz, size, write_offset, hse->input_size);
    if (cp_sz == rem) {
        TRACE("-- internal buffer is now full\n");
...
HSE_poll_res heatshrink_encoder_poll(heatshrink_encoder *hse,
    ...
    if (out_buf_size == 0) {
        TRACE("-- MISUSE: output buffer size is 0\n");
    ...
    *output_size = 0;
    ...
    while (1) {
        TRACE("polling, state %u (%s), flags 0x%02x\n",
...
HSE_finish_res heatshrink_encoder_finish(heatshrink_encoder *hse) {
    ...
    TRACE("-- setting is_finishing flag\n");
...
static HSE_state st_step_search(heatshrink_encoder *hse) {
    ...
    TRACE("## step_search, scan @ +%d (%d/%d), input size %d\n",
...
static HSE_state st_yield_br_index(heatshrink_encoder *hse,
        output_info *oi) {
    if (can_take_byte(oi)) {
        TRACE("-- yielding backref index %u\n", hse->match_pos);
...
static HSE_state st_yield_br_length(heatshrink_encoder *hse,
        output_info *oi) {
    if (can_take_byte(oi)) {
        TRACE("-- yielding backref length %u\n", hse->match_length);
        if (push_outgoing_bits(hse, oi) > 0) {
...
static HSE_state st_flush_bit_buffer(heatshrink_encoder *hse,
    ...
    if (hse->bit_index == 0x80) {
        ...
        TRACE("-- flushing remaining byte (bit_index == 0x%02x)\n", hse->bit_index);
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start,
        uint16_t end, const uint16_t maxlen, uint16_t *match_length) {
    TRACE("-- scanning for match of buf[%u:%u] between buf[%u:%u] (max %u bytes)\n",
    ...
    for (int16_t pos=end - 1; pos - (int16_t)start >= 0; pos--) {
        ...
        if ((pospoint[match_maxlen] == needlepoint[match_maxlen])
            ...
            for (len=1; len<maxlen; len++) {
                if (0) {
                    TRACE("  --> cmp buf[%d] == 0x%02x against %02x (start %u)\n",
...
static uint8_t push_outgoing_bits(heatshrink_encoder *hse, output_info *oi) {
    ...
    if (count > 0) {
        TRACE("-- pushing %d outgoing bits: 0x%02x\n", count, bits);
...
static void push_bits(heatshrink_encoder *hse, uint8_t count, uint8_t bits,
        output_info *oi) {
    ASSERT(count <= 8);
    TRACE("++ push_bits: %d bits, input of 0x%02x\n", count, bits);
    ...
    if (count == 8 && hse->bit_index == 0x80) {
        ...
        for (int i=count - 1; i>=0; i--) {
            ...
            if (0) {
                TRACE("  -- setting bit %d at bit index 0x%02x, byte => 0x%02x\n",
                    bit ? 1 : 0, hse->bit_index, hse->current_byte);
            }
            hse->bit_index >>= 1;
            if (hse->bit_index == 0x00) {
                hse->bit_index = 0x80;
                TRACE(" > pushing byte 0x%02x\n", hse->current_byte);
                oi->buf[(*oi->output_size)++] = hse->current_byte;
                hse->current_byte = 0x00;
            }
        }
...
static void push_literal_byte(heatshrink_encoder *hse, output_info *oi) {
    ...
    TRACE("-- yielded literal byte 0x%02x ('%c') from +%d\n",
